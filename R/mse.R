...
li_lahiri <- function(framework, sigmau2, combined_data, method) {
    ...
    if (!all(framework$obs_dom == TRUE)) {
      MSE_data$MSE[framework$obs_dom == FALSE] <- prasad_rao$MSE[framework$obs_dom == FALSE]
      MSE_data$ind[framework$obs_dom == FALSE] <- 1
    ...
    cat("Please note that only for in-sample-domains a correction following
        ...
        the MSE for the regression-synthetic estimator is returned without
        any correction. For the reference see help(FH_AK).")
    }
...
analytical_mse <- function(framework, sigmau2, eblup, combined_data,
                           method) {
